...
- name: "Rosario Michael Piro"
  affiliation: Dep. of Computer Science, Freie Universität Berlin, Germany
...
<https://www.synapse.org/#!Synapse:syn12009743>). For version 3, only Single
Base Substitution (SBS) signatures can be used.
...
the lack of the redundant 3rd annotation column in version 3.)
...
our papers; Krüger and Piro, 2017, 2018). In newer versions,
...
the very same functions described in Section 3.1.5 (see above).
...
the following example reads the tumor genomes provided with this package using
the Alexandrov model (see Section 3.2.1) and plots the mutation frequencies of
the first genome:
...
programming. For details, see our paper (Krüger and Piro, 2018).
